...
\begin{abstract}
Experimento: Comparação de desempenho de duas configurações de um
...
\CommentTok{# FOR INSTANCE: set dim = 10}
...
\NormalTok{stopcrit <-}\StringTok{ }\KeywordTok{list}\NormalTok{(}\DataTypeTok{names =} \StringTok{"stop_maxeval"}\NormalTok{, }\DataTypeTok{maxevals =} \DecValTok{5000} \OperatorTok{*}\StringTok{ }\NormalTok{dim, }\DataTypeTok{maxiter =} \DecValTok{100} \OperatorTok{*}\StringTok{ }\NormalTok{dim)}
...
\NormalTok{## Config 2}
\NormalTok{recpars2 <-}\StringTok{ }\KeywordTok{list}\NormalTok{(}\DataTypeTok{name =} \StringTok{"recombination_eigen"}\NormalTok{, }\DataTypeTok{othername =} \StringTok{"recombination_bin"}\NormalTok{, }\DataTypeTok{cr =} \FloatTok{0.9}\NormalTok{) }
...
\NormalTok{## Config 2}
\NormalTok{recpars2 <-}\StringTok{ }\KeywordTok{list}\NormalTok{(}\DataTypeTok{name =} \StringTok{"recombination_blxAlphaBeta"}\NormalTok{, }\DataTypeTok{alpha =} \FloatTok{0.1}\NormalTok{, }\DataTypeTok{beta =} \FloatTok{0.4}\NormalTok{)}
...
\NormalTok{## Config 2}
\NormalTok{recpars1 <-}\StringTok{ }\KeywordTok{list}\NormalTok{(}\DataTypeTok{name =} \StringTok{"recombination_npoint"}\NormalTok{, }\DataTypeTok{N =}\NormalTok{ dim }\OperatorTok{/}\StringTok{ }\DecValTok{2}\NormalTok{)}
\NormalTok{mutpars1 <-}\StringTok{ }\KeywordTok{list}\NormalTok{(}\DataTypeTok{name =} \StringTok{"mutation_rand"}\NormalTok{, }\DataTypeTok{f =} \FloatTok{2.2}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\noindent onde \emph{mutparsX} e \emph{recparsX} devem ser substituídos
pelas variáveis apropriadas (e.g., \emph{mutpars1}, \emph{mutpars2}
etc.).
