\documentclass[11pt,printwatermark=false]{pinp}
...
diferentes configurações deste algoritmo em seu desempenho para um dado
problema. Para isso, o mesmo implementou o algoritmo e diversos
operadores de forma padronizada, na forma do pacote ExpDE
(\url{https://cran.r-project.org/web/packages/ExpDE/})
\citep{Campelo2016}, cuja versão de desenvolvimento encontra-se
disponível no Github
(\url{https://github.com/fcampelo/ExpDE/tree/devel}).

\subsection{Atividades}\label{atividades}

Como forma de análise preliminar deste problema, cada equipe terá como
tarefa a comparação experimental de algumas configurações em um único
problema de teste. O objetivo deste estudo é responder às perguntas:
...
\tightlist
\item
  Mínima diferença de importância prática entre qualquer par de
  algoritmos (padronizada, em termos do coeficiente \emph{d} de Cohen):
  (\(d^* = \delta^*/\sigma\)) = 0.25
\item
...
Para instalar e carregar o pacote no computador, os seguintes comandos
...
As configurações que deverão ser comparadas por cada grupo passaram por
uma etapa anterior de ajuste de parâmetros. Suas definições são dadas
por:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Equipe Iuri, Henrique, Rodrigo}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Equipe Hugo, Lucas, Rafael, Victor}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Equipe Monica, Leticia Mayra, Mateus }
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Equipe Ezequiel, Lucas, Ricardo}

\NormalTok{## Config 1}
\NormalTok{recpars1 <-}\StringTok{ }\KeywordTok{list}\NormalTok{(}\DataTypeTok{name =} \StringTok{"recombination_onepoint"}\NormalTok{, }\DataTypeTok{K =} \DecValTok{17}\NormalTok{)}
\NormalTok{mutpars1 <-}\StringTok{ }\KeywordTok{list}\NormalTok{(}\DataTypeTok{name =} \StringTok{"mutation_best"}\NormalTok{, }\DataTypeTok{f =} \FloatTok{2.4}\NormalTok{)}
\NormalTok{popsize1 <-}\StringTok{ }\DecValTok{225}

\NormalTok{## Config 2}
\NormalTok{recpars2 <-}\StringTok{ }\KeywordTok{list}\NormalTok{(}\DataTypeTok{name =} \StringTok{"recombination_pbest"}\NormalTok{, }\DataTypeTok{cr =} \FloatTok{0.25}\NormalTok{)}
\NormalTok{mutpars2 <-}\StringTok{ }\KeywordTok{list}\NormalTok{(}\DataTypeTok{name =} \StringTok{"mutation_rand"}\NormalTok{, }\DataTypeTok{f =} \FloatTok{3.5}\NormalTok{)}
\NormalTok{popsize2 <-}\StringTok{ }\DecValTok{325}

\NormalTok{## Config 3}
\NormalTok{recpars3 <-}\StringTok{ }\KeywordTok{list}\NormalTok{(}\DataTypeTok{name =} \StringTok{"recombination_sbx"}\NormalTok{, }\DataTypeTok{eta =} \DecValTok{90}\NormalTok{)}
\NormalTok{mutpars3 <-}\StringTok{ }\KeywordTok{list}\NormalTok{(}\DataTypeTok{name =} \StringTok{"mutation_best"}\NormalTok{, }\DataTypeTok{f =} \FloatTok{4.5}\NormalTok{)}
\NormalTok{popsize3 <-}\StringTok{ }\DecValTok{200}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Equipe Eduardo, Guilherme, Paulo, Franklin}
...
\NormalTok{## Config 3}
\NormalTok{recpars3 <-}\StringTok{ }\KeywordTok{list}\NormalTok{(}\DataTypeTok{name =} \StringTok{"recombination_linear"}\NormalTok{)}
\NormalTok{mutpars3 <-}\StringTok{ }\KeywordTok{list}\NormalTok{(}\DataTypeTok{name =} \StringTok{"mutation_rand"}\NormalTok{, }\DataTypeTok{f =} \FloatTok{1.5}\NormalTok{)}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Equipe Rafael, Marcus, Paulo, Elany}
...
\NormalTok{## Config 2}
\NormalTok{recpars2 <-}\StringTok{ }\KeywordTok{list}\NormalTok{(}\DataTypeTok{name =} \StringTok{"recombination_mmax"}\NormalTok{, }\DataTypeTok{lambda =} \FloatTok{0.25}\NormalTok{)}
\NormalTok{mutpars2 <-}\StringTok{ }\KeywordTok{list}\NormalTok{(}\DataTypeTok{name =} \StringTok{"mutation_best"}\NormalTok{, }\DataTypeTok{f =} \DecValTok{4}\NormalTok{)}
\NormalTok{popsize2 <-}\StringTok{ }\DecValTok{375}

\NormalTok{## Config 3}
\NormalTok{recpars3 <-}\StringTok{ }\KeywordTok{list}\NormalTok{(}\DataTypeTok{name =} \StringTok{"recombination_pbest"}\NormalTok{, }\DataTypeTok{cr =} \FloatTok{0.25}\NormalTok{)}
\NormalTok{mutpars3 <-}\StringTok{ }\KeywordTok{list}\NormalTok{(}\DataTypeTok{name =} \StringTok{"mutation_rand"}\NormalTok{, }\DataTypeTok{f =} \FloatTok{3.5}\NormalTok{)}
\NormalTok{popsize3 <-}\StringTok{ }\DecValTok{325}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Equipe Marcelo, Brayan, William, André}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Equipe Letícia Resende, Marcelo, André}
...
Cada observação individual do desempenho do algoritmo com um dado
operador pode ser obtida através dos comandos abaixo:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Run algorithm on problem:}
\NormalTok{out <-}\StringTok{ }\KeywordTok{ExpDE}\NormalTok{(}\DataTypeTok{popsize  =}\NormalTok{ popsizeX, }
             \DataTypeTok{mutpars  =}\NormalTok{ mutparsX, }
             ...
             \DataTypeTok{showpars =} \KeywordTok{list}\NormalTok{(}\DataTypeTok{show.iters =} \StringTok{"dots"}\NormalTok{,}
                             \DataTypeTok{showevery  =} \DecValTok{20}\NormalTok{))}
...
``\textbf{{[}EEE933\_2017-2\_EC03{]} Nome\_da\_equipe}'' (sem as aspas).
A data-limite para o recebimento dos arquivos é \textbf{terça-feira
(07/11) às 11:00h}
